...
switch ($_POST['action']) {
    ...
    case 'send_cmd':
        if(!(isset($_POST['cmd_tpye']) and isset($_POST['target'])))die("Bad command!");
        $_POST['cmd_tpye'] = ($_POST['cmd_tpye'] == 'grant'? 1 : 2);
        $sql = "SELECT lock FROM user_lock WHERE username='". $_POST['user'] . "' AND lockname='". $_POST['target']. "'";
        $re = $db->query($sql);
        if($re.num_rows<1){
            $result['status'] = 1;
        ...
        $sql = "INSERT INTO cmds (username, lockname, type)VALUES('". $_POST['user']. "','". $_POST['target']). "',". $_POST['cmd_tpye']. ")";
        if(!$db->query($sql)){
            $result['status'] = 1;
